...
const SIGNPOSTING_FORBIDDEN: { pattern: RegExp; replace: string }[] = [
  ...
  { pattern: /\b(solid|strong|good|meritorious) case\b/gi, replace: "possible route" },
  { pattern: /\blikely entitled\b/gi, replace: "may need to check entitlement" },
  { pattern: /\byou have a claim\b/gi, replace: "a claim may be worth checking" },
  { pattern: /\bconstitute[sd]? a (?:solid |strong |good |meritorious )?(?:legal )?case for\b/gi, replace: "may raise a question of" },
  { pattern: /\b(guarantee[ds]?|guaranteed|guaranty)\b/gi, replace: "" },
  { pattern: /\b(\d+%\s*(success|win|chance|likely|probability))\b/gi, replace: "" },
