...
export default function FailedSearchesAdminClient() {
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const res = await fetch("/api/admin/search-quality?action=failed", { cache: "no-store" });
  ...
  const runReplay = async () => {
    ...
    const res = await fetch(
      `/api/admin/search-quality?action=replay&id=${encodeURIComponent(replayId.trim())}`,
      { cache: "no-store" },
    );
